@extends('BlogLayouts.bloglayouts')

@section('content')
<section class="article-post-inner">
        <div class="container">
            <div class="row">
                <div class="col-sm-8">                    
                    <div class="articale-list">
                            @foreach ($posts as $post)
                        <div class="post-style2 wow fadeIn  " data-wow-duration="1s">
                            <div class="ImgBx">
                                    <a href="/blog/{{$post->id }}"><img class="img" src="{{asset('storage/'.$post->Images)}}" alt="alt"></a>

                            </div>
                            <div class="post-style2-detail">
                                <h3><a href="blog/{{$post->id }}" title="{{$post->Title}}">{{$post->Title}}</a></h3>
                                <div class="date">
                                    <ul>
                                        <li><img src="{{ asset('storage/'.$post->Images) }}" class="img-responsive" alt=""></li>
                                        <li>By <a title="{{$post->Author}}" href="blog/{{$post->id }}"><span>{{$post->Author}}</a> --</li>
                                        <li><a title="{{$post->created_at}}" href="blog/{{$post->id }}">{{$post->created_at}}</li>
                                    </ul>
                                </div>
                                <p>{{Str::limit($post->Description, 50)}}...</p>
                                <a href="blog/{{$post->id }}"  type="button" class="btn btn-style">Read more</a>
                                
                            </div>
                        </div>
                        @endforeach
                    </div>
                     <div class="col-sm-12">
                         <ul class="pagination">
                             <li>
                                 {{-- <a href="#" class="prev"> --}}
                                     {{-- <i class="pe-7s-angle-left"> --}}
                            {{ $posts->links() }}  
                                     </i>
                                 </a>
                             </li>
                         </ul>
                     </div>
                </div>
                 <aside class="col-sm-4 left-padding">
                     
                     <!-- /.social icon -->
                     <div class="tab-inner">
                         <ul class="tabs">
                             <li><a href="#">Categories</a></li>
                         </ul>
                         <hr>
                         <!-- tabs -->
         @foreach ($cats as $cat)
                         
                         <div class="tab_content">
                             <div class="tab-item-inner">
                                 <div class="box-item wow fadeIn" data-wow-duration="1s">
                                     <div class="img-thumb">
                                         <a href="categories/{{$cat->id  }}" rel="bookmark"><img class="entry-thumb"  src="{{'storage/'.$cat->Images}}" alt="" height="80" width="90"></a>
                                     </div>
                                     <div class="item-details">
                                         <h6 class="sub-category-title bg-color-1">
                                         <a href="categories/{{$cat->id  }}">{{$cat->Categories}}</a>
                                             </h6>
                                         <h3 class="td-module-title"><a href="categories/{{$cat->id  }}">{{$cat->Title}}</a></h3>
                                         <div class="post-editor-date">
                                             <!-- post date -->
                                             <div class="post-date">
                                                 <i class="pe-7s-clock"></i> 
                                                 {{ $cat->created_at }}
                                             </div>
                                             <!-- post comment -->
                                             {{-- <div class="post-author-comment"><i class="pe-7s-comment"></i> 13 </div> --}}
                                         </div>
                                     </div>
                                 </div>
                                 @endforeach
                                
                             </div>
                             <!-- / tab item -->
                             <div class="tab-item-inner">
                                 <div class="box-item">
                                     <div class="img-thumb">
                                         <a href="#" rel="bookmark"><img class="entry-thumb"  src="{{asset('blog/images/popular_news_01.jpg')}}" alt="" height="80" width="90"></a>
                                     </div>
                                     <div class="item-details">
                                         <h6 class="sub-category-title bg-color-5">
                                                 <a href="#">BUSINESS</a>
                                             </h6>
                                         <h3 class="td-module-title"><a href="#">It is a long established fact that a reader will</a></h3>
                                         <div class="post-editor-date">
                                             <!-- post date -->
                                             <div class="post-date">
                                                 <i class="pe-7s-clock"></i> Oct 6, 2016
                                             </div>
                                             <!-- post comment -->
                                             <div class="post-author-comment"><i class="pe-7s-comment"></i> 13 </div>
                                         </div>
                                     </div>
                                 </div>
                                 <div class="box-item">
                                     <div class="img-thumb">
                                         <a href="#" rel="bookmark"><img class="entry-thumb"  src="{{asset('blog/images/popular_news_02.jpg')}}" alt="" height="80" width="90"></a>
                                     </div>
                                     <div class="item-details">
                                         <h6 class="sub-category-title bg-color-2">
                                                 <a href="#">TECHNOLOGY </a>
                                             </h6>
                                         <h3 class="td-module-title"><a href="#">The generated Lorem Ipsum is therefore</a></h3>
                                         <div class="post-editor-date">
                                             <!-- post date -->
                                             <div class="post-date">
                                                 <i class="pe-7s-clock"></i> Oct 6, 2016
                                             </div>
                                             <!-- post comment -->
                                             <div class="post-author-comment"><i class="pe-7s-comment"></i> 13 </div>
                                         </div>
                                     </div>
                                 </div>
                                 <div class="box-item">
                                     <div class="img-thumb">
                                         <a href="#" rel="bookmark"><img class="entry-thumb"  src="{{asset('blog/images/popular_news_03.jpg')}}" alt="" height="80" width="90"></a>
                                     </div>
                                     <div class="item-details">
                                         <h6 class="sub-category-title bg-color-3">
                                                 <a href="#">HEALTH</a>
                                             </h6>
                                         <h3 class="td-module-title"><a href="#">The standard chunk of Lorem Ipsum used since</a></h3>
                                         <div class="post-editor-date">
                                             <!-- post date -->
                                             <div class="post-date">
                                                 <i class="pe-7s-clock"></i> Oct 6, 2016
                                             </div>
                                             <!-- post comment -->
                                             <div class="post-author-comment"><i class="pe-7s-comment"></i> 13 </div>
                                         </div>
                                     </div>
                                 </div>
                                 <div class="box-item">
                                     <div class="img-thumb">
                                         <a href="#" rel="bookmark"><img class="entry-thumb"  src="{{asset('blog/images/popular_news_04.jpg')}}" alt="" height="80" width="90"></a>
                                     </div>
                                     <div class="item-details">
                                         <h6 class="sub-category-title bg-color-4">
                                                 <a href="#">FASHION</a>
                                             </h6>
                                         <h3 class="td-module-title"><a href="#">Lorem Ipum therefore always free from</a></h3>
                                         <div class="post-editor-date">
                                             <!-- post date -->
                                             <div class="post-date">
                                                 <i class="pe-7s-clock"></i> Oct 6, 2016
                                             </div>
                                             <!-- post comment -->
                                             <div class="post-author-comment"><i class="pe-7s-comment"></i> 13 </div>
                                         </div>
                                     </div>
                                 </div>
                             </div>
                             <!-- / tab item -->
                         </div>
                         <!-- / tab_content -->
                     </div>
                     <!-- / tab -->
                     <div class="banner-add">
                         <!-- add -->
                         <span class="add-title">- Advertisement -</span>
                         <a href="#"><img  src="{{asset('blog/images/ad-banner.jpg')}}" class="img-responsive center-block" alt=""></a>
                     </div>
                     <!-- comments -->
                     <div class="latest-comments-inner">
                         <h3 class="category-headding ">LATEST COMMENT</h3>
                         <div class="headding-border"></div>
                         <!-- latest comment post -->
                         <div class="post-style2 latest-com">
                             {{-- <img  src="{{asset('blog/images/comment-01.jpg')}}" alt=""> --}}
                             
                             <div class="latest-com-detail">
                             @foreach($Comm as $comment)
                                 
                                 <h5><a href="/blog/{{ $comment->Comment_id }}" title="">{{$comment->Comment}}</a></h5>
                                 {{-- <span>Nec sagittis sem nibh dictionary...</span>
                                 <p>Proin gravida nibh vel velit auctor aliquet. </p> --}}
                             @endforeach
                                 
                             </div>
                         </div>
                         <!-- latest comment post -->
                         {{-- <div class="post-style2 latest-com">
                             <img  src="{{asset('blog/images/comment-02.jpg')}}" alt="">
                             <div class="latest-com-detail">
                                 <h5><a href="#" title="">It uses a dictionary of over</a></h5>
                                 <span>Nec sagittis sem nibh dictionary...</span>
                                 <p>Proin gravida nibh vel velit auctor aliquet. </p>
                             </div>
                         </div> --}}
                     </div>
                     <!-- slider widget -->
                     <div class="widget-slider-inner">
                         
                         <div id="widget-slider" class="owl-carousel owl-theme">
                     @foreach($ca as $ca)
                             
                             <!-- widget item -->
                             <div class="item">
                                 <a href="categories/{{ $ca->id }}"><img  src="{{'storage/'.$ca->Images}}" alt=""></a>
                                 <h4><a href="categories/{{ $ca->id }}">{{$ca->Title}}</a></h4>
                                 <div class="date">
                                     <ul>
                                         <li>By<a title="{{$ca->Author}}" href="categories/{{ $ca->id }}"><span>{{$ca->Author}}</span></a> --</li>
                                         <li><a title="{{$ca->created_at}}" href="categories/{{ $ca->id }}">{{$ca->created_at}}</a></li>
                                     </ul>
                                 </div>
                                 <p>{{$ca->Content}}</p>
                             </div>
                             @endforeach
                            
                 </aside>
             </div>
                     <div class="col-sm-12">
                         <div class="banner">
                             <img   src="{{asset('blog/images/top-bannner2.jpg')}}" class="img-responsive center-block" alt="">
                         </div>
                     </div>
                 </div>
             </div>
         </div>
         
         
@endSection